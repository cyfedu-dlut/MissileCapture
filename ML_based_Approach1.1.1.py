...
class MissileStrikeAnalyzer:    #导弹打击分析主类（重要参数:video_path）
    def __init__(self, video_path):     #初始化视频参数和检测器（关键技术：视频捕获、背景建模；重要参数：history=500, varThreshold=16）
        ...
        self.explosion_change_ratio_threshold = 0.2
        self.min_roi_pixels = 1024  # 32*32
        # 爆炸结束检测参数
        self.stability_threshold = 15  # 连续稳定帧数阈值，增加到30帧以确保真正稳定
    ...
    def detect_missile(self, frame, fg_mask):   #检测并跟踪导弹（完成的核心步骤：碰撞冷却检测：跳过爆炸后的无效检测。​前景区域提取：通过二值化和形态学操作去噪。​轮廓定位：为后续导弹筛选（面积、长宽比、轨迹连续性检查）提供候选区域）
        ...
        # 根据分辨率调整形态学核大小
        kernel_size = int(max(3, frame.shape[1] // 200))  # 每200像素宽度用1个核单位
        ...
        for contour in contours:
            ...
            min_area = int((width * height) * 0.0001)  # 0.01% of frame area
            max_area = int((width * height) * 0.05)   # 5% of frame area
            # 过滤小的噪点和大的区域
            if min_area < area < max_area:  # 调整这些阈值以适应导弹大小（50和2000是固定值，可能不适用于不同分辨率或场景的视频）
                ...
                if 0.1 < aspect_ratio < 0.5:
                    ...
                    if len(self.missile_trajectory) > 0:
                        ...
                        # 如果移动距离太远，可能不是同一个导弹
                        fps = self.fps
                        max_speed_px_per_sec = 1000  # 假设最大速度
                        max_distance = (max_speed_px_per_sec / fps) * 1.5  # 1.5倍容差
    ...
    def detect_explosion(self, frame):  #检测爆炸和烟尘发生（关键技术：HSV颜色空间分析；重要参数：brightness_threshold=30）
        ...
        if total_pixels < self.min_roi_pixels:
            scale_factor = (self.min_roi_pixels - total_pixels) / self.min_roi_pixels
            base_threshold + scale_factor * 0.15
        # 如果变化比例超过阈值，认为爆炸发生
