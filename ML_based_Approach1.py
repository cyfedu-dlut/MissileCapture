...
class MissileStrikeAnalyzer:    #导弹打击分析主类（重要参数:video_path）
    ...
    def select_target_roi(self):    #选择目标区域（重要参数：showCrosshair=True）
        ...
        # 显示第一帧并让用户选择目标区域
        cv2.namedWindow("选择目标区域")
        cv2.imshow("选择目标区域", frame_with_text)
        self.target_roi = cv2.selectROI("选择目标区域", frame_with_text, fromCenter=False, showCrosshair=True)
    ...
    def format_time(self, seconds): #辅助方法：时间格式化（将秒数格式化为时:分:秒.毫秒格式）
        """将秒数格式化为时:分:秒.毫秒格式"""
        if seconds is None:
            return "未检测到"
        ...
        '''问题总结
            ​超过24小时显示错误
            timedelta.seconds 不包含天数转换的小时数，导致小时数上限为23。
            ​依赖timedelta引入冗余
            直接数学计算更高效且避免潜在错误。
            ​毫秒处理精度问题
            截断可能导致显示误差，建议四舍五入。
            ...
            def format_time(self, seconds):
                ...
                ms = int(round((seconds - total_seconds) * 1000)) % 1000  # 四舍五入毫秒
                hours = total_seconds // 3600
                minutes = (total_seconds % 3600) // 60
                seconds_remainder = total_seconds % 60
                return f"{hours:02d}:{minutes:02d}:{seconds_remainder:02d}.{ms:03d}"'''
    ...
    def create_display_frame(self, current_frame):  #生成双屏显示画面（使用4个关键帧显示区）
        """创建显示帧，包括当前视频帧和关键帧"""
        
        '''功能：创建一个全黑的RGB画布，尺寸为 self.display_height（高）x self.display_width（宽）。
        ​潜在问题：
        ​硬编码通道数：假设输入帧始终为3通道（BGR），若处理灰度图会出错。
        ​尺寸匹配问题：若 current_frame 尺寸与 self.frame_height/width 不匹配，后续拼接会失败。
        ​优化建议：
        # 动态获取输入帧尺寸
        ...
        '''功能：将当前视频帧放置在画布左侧区域。
        ​关键问题：
        ​无缩放处理：若 current_frame 尺寸与画布预留区域不匹配，会导致图像截断或留黑边。
        ​优化方向：
        # 缩放当前帧以适应左侧区域
        resized_current = cv2.resize(current_frame, (self.frame_width, self.frame_height))
        display_frame[0:self.frame_height, 0:self.frame_width] = resized_current'''
        # 将当前帧放在左侧
        display_frame[0:self.frame_height, 0:self.frame_width] = current_frame
        
        '''功能：计算每个关键帧的显示尺寸（高度为原帧1/4，宽度相同），为右侧垂直排列4个关键帧做准备。
        ​潜在问题：
        ​尺寸不合理：若原帧高度较小（如720p），关键帧高度可能过小（如180像素），导致显示模糊。
        ​宽高比失真：未保持关键帧原始宽高比，直接拉伸可能变形。
        ​优化建议：
        # 根据显示区域动态计算关键帧尺寸（保持宽高比）
        key_frame_aspect_ratio = self.key_frames[0].shape[1] / self.key_frames[0].shape[0]  # 假设有关键帧
        key_frame_height = min(self.display_height // 4, int(self.frame_width / key_frame_aspect_ratio))
        key_frame_width = int(key_frame_height * key_frame_aspect_ratio)'''
        # 确定关键帧的显示位置
        key_frame_height = self.frame_height // 4  # 调整为4个场景
        key_frame_width = self.frame_width
        
        '''功能：在左右区域顶部添加中文标题。
        ​关键问题：
        ​位置硬编码：(10, 30) 和 (self.frame_width + 10, 30) 缺乏动态适配，可能在不同分辨率下错位。
        ​文字可读性：白色文字在浅色背景（如天空）上可能难以辨识。
        ​优化建议：
        # 添加半透明背景框
        text = "当前视频"
        text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
        cv2.rectangle(display_frame, (5, 5), (text_size[0]+15, text_size[1]+20), (0,0,0), -1)
        display_frame = self.put_chinese_text(display_frame, text, (10, 30), 36, (255,255,255))'''
        # 添加标签
        display_frame = self.put_chinese_text(display_frame, "当前视频", (10, 30), 36, (255, 255, 255))
        display_frame = self.put_chinese_text(display_frame, "关键事件", (self.frame_width + 10, 30), 36, (255, 255, 255))
        
        '''功能：在左侧区域下方显示检测状态，使用黄色文字。
        ​关键问题：
        ​信息覆盖：若 status_y 位置低于 self.frame_height，状态信息会覆盖在视频画面上。
        ​颜色对比度：黄色 ((255,255,0)) 在白色背景下难以阅读。
        ​优化建议：
        # 将状态信息放置在左侧视频区域下方
        status_y = self.frame_height + 20  # 视频区域下方留20像素空白
        # 使用高对比度颜色（如青蓝色）
        display_frame = self.put_chinese_text(display_frame, status_text, (10, status_y), 24, (255, 255, 0))'''
        # 显示状态信息
        status_y = 70
        ...
        display_frame = self.put_chinese_text(
            display_frame,
            f"导弹: {missile_status} | 碰撞: {impact_status} | 爆炸: {explosion_status} | 爆炸结束: {explosion_end_status}",
            (10, status_y),
            24,
            (255, 255, 0)
        )

        # 显示关键帧及其时间戳 - 第一个场景：导弹出现
        if self.key_frames["missile_appearance"] is not None:
            # 缩放关键帧到显示区域尺寸
            resized_frame = cv2.resize(self.key_frames["missile_appearance"],
                                       (key_frame_width, key_frame_height))
            # 将关键帧放置在右侧第一个区域（Y坐标0~key_frame_height）
            display_frame[0:key_frame_height, self.frame_width:self.frame_width + key_frame_width] = resized_frame
            # 添加绿色时间戳标签（位置在关键帧底部）
            time_str = self.format_time(self.key_frames_time["missile_appearance"])
            display_frame = self.put_chinese_text(
                display_frame,
                f"1. 导弹出现 - {time_str}",
                (self.frame_width + 10, key_frame_height - 30),
                24,
                (0, 255, 0) # 绿色表示已检测
            )
        else:
            # 如果没有检测到，显示占位符,灰色占位符表示未检测
            display_frame = self.put_chinese_text(
                display_frame,
                "1. 导弹出现 - 等待检测",
                (self.frame_width + 10, key_frame_height - 30),
        ...
        '''功能：显示导弹出现的关键帧及时间，未检测时显示灰色占位文本。
        ​潜在问题：
        ​图像变形：cv2.resize 未保持宽高比，关键帧可能被拉伸。
        ​标签位置冲突：key_frame_height - 30 可能超出关键帧区域（若关键帧高度小于30像素）。
        ​优化建议：
        # 保持宽高比缩放,后面3个场景有一样的问题
        aspect_ratio = self.key_frames["missile_appearance"].shape[1] / self.key_frames["missile_appearance"].shape[0]
        resized_height = key_frame_height
        resized_width = int(resized_height * aspect_ratio)
        resized_frame = cv2.resize(self.key_frames["missile_appearance"], (resized_width, resized_height))'''

            # 第二个场景：导弹接触目标
        if self.key_frames["missile_impact"] is not None:
            resized_frame = cv2.resize(self.key_frames["missile_impact"],
                                       (key_frame_width, key_frame_height))
            display_frame[key_frame_height:2 * key_frame_height,
            self.frame_width:self.frame_width + key_frame_width] = resized_frame
            # 添加时间戳和标签
            time_str = self.format_time(self.key_frames_time["missile_impact"])
            display_frame = self.put_chinese_text(
                display_frame,
                f"2. 导弹接触目标 - {time_str}",
                (self.frame_width + 10, 2 * key_frame_height - 30),
                24,
                (0, 255, 0)
            )
        else:
            ...
            # 第三个场景：爆炸发生
        if self.key_frames["explosion"] is not None:
            resized_frame = cv2.resize(self.key_frames["explosion"],
                                       (key_frame_width, key_frame_height))
            display_frame[2 * key_frame_height:3 * key_frame_height,
            self.frame_width:self.frame_width + key_frame_width] = resized_frame
            # 添加时间戳和标签
            time_str = self.format_time(self.key_frames_time["explosion"])
            display_frame = self.put_chinese_text(
                display_frame,
                f"3. 爆炸发生 - {time_str}",
                (self.frame_width + 10, 3 * key_frame_height - 30),
                24,
                (0, 255, 0)
            )
        else:
            ...
            # 第四个场景：爆炸结束
        if self.key_frames["explosion_end"] is not None:
            resized_frame = cv2.resize(self.key_frames["explosion_end"],
                                       (key_frame_width, key_frame_height))
            display_frame[3 * key_frame_height:4 * key_frame_height,
            self.frame_width:self.frame_width + key_frame_width] = resized_frame
            # 添加时间戳和标签
            time_str = self.format_time(self.key_frames_time["explosion_end"])
            display_frame = self.put_chinese_text(
                display_frame,
                f"4. 爆炸结束 - {time_str}",
                (self.frame_width + 10, 4 * key_frame_height - 30),
                24,
                (0, 255, 0)
            )
        else:
        ...
        '''关键问题：
        ​显示区域不足：若 self.display_height < 4*key_frame_height，部分内容会被截断。
        ​优化建议：
        # 动态计算最大显示高度
        max_display_height = self.display_height - status_bar_height  # 预留状态栏高度
        key_frame_height = max_display_height // 4'''

        # 添加当前时间
        current_time = self.format_time(self.frame_counter / self.fps)
        display_frame = self.put_chinese_text(
            display_frame,
            f"当前时间: {current_time}",
    ...
    def process_video(self, output_path=None):  #处理视频主循环（关键技术：帧处理链式调用）
        ...
        while True:
            ...
            多线程分离显示和计算逻辑。'''
            # 显示结果
            cv2.imshow("导弹打击目标分析", display_frame)
            # 写入视频
            if output_path:
...
class MissileStrikeAnalyzerCamera(MissileStrikeAnalyzer):
    ...
    def process_video(self, output_path=None):
        ...
        while True:
            ...
            # 显示结果
            cv2.imshow("导弹打击目标分析 - 摄像头", display_frame)
