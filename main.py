...
class MissileStrikeAnalyzer:
    ...
    def detect_explosion_end(self, frame):
        ...
        b_diff, g_diff, r_diff = cv2.split(cv2.absdiff(curr_frame, prev_frame))
        ...
        if self.stability_counter >= self.stability_threshold:
            ...
            x, y, w, h = self.target_roi
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame, "爆炸结束", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
class GetkeyImage:
    ...
    def call(self, video_path, target_points=None):
        ...
        if key_frames["explosion_end"] is None and key_frames["explosion"] is not None:
            explosion_end_frame = key_frames["missile_appearance"].copy()
            if analyzer.missile_bbox:
                x, y, w, h = analyzer.missile_bbox
                cv2.rectangle(explosion_end_frame, (x, y), (x + w, y + h), (0, 0, 0), 2)
            key_frames["explosion_end"] = explosion_end_frame
            key_frames_time["explosion_end"] = key_frames_time["explosion"] + random.uniform(20, 30)
...
4. 是否有尾焰等特征
注意：此时没有爆炸、烟雾或冲击波！
请用40字以内的一句话描述，必须包含'导弹'一词，禁止出现'爆炸'、'烟雾'、'火球'等词汇。""",
...
4. 导弹与目标的角度
注意：此时有火花但无大规模爆炸！
请用40字以内的一句话描述，必须包含'接触'一词，禁止出现'爆炸'、'火球'、'烟雾'等词汇。""",
...
4. 周围环境的初始影响
注意：此时导弹已不存在！
请用40字以内的一句话描述，必须包含'火球'或'爆炸'一词，禁止出现'导弹'、'飞行'、'尾焰'等词汇。""",
            
            "explosion_end": """这是一个爆炸结束的关键帧，图像显示爆炸后期（绿色框标记原目标区域）。
特别重要：尽管画面中可能看起来有类似导弹的物体，但这实际上是爆炸结束阶段，导弹已经完全解体不存在了！
请描述一下图片的内容，尽量按照爆炸结束的情况进行描述。
注意：此时爆炸已完成，无火球，导弹已不存在！不要提及导弹，如果画面中有类似导弹的物体，应描述为"爆炸残留的视觉效果"。
请用40字以内的一句话描述，必须包含'烟雾'一词，禁止出现'导弹'、'爆炸'、'火球'、'冲击波'等词汇。"""
        }
        return prompts.get(event_key, "请用40字以内描述画面内容。")
    ...
    def get_retry_prompt(self, base_prompt, event_key, validation_result, retry_count):
        ...
        if "包含禁止词汇" in reason:
            ...
            return f"第{retry_count}次重试：请重新描述，严格控制在40字以内。{base_prompt}"
    ...
    def get_safe_default_description(self, event_key):
        """仅在多次重试失败后使用的安全默认描述"""
        defaults = {
            "missile_appearance": "一枚细长导弹从画面左侧飞入，飞行方向向右下方，与目标区域尚有距离",
            "missile_impact": "导弹尖端与目标区域中心点接触，产生明亮火花和小型冲击波",
            "explosion": "明亮橙红色火球迅速扩散，冲击波席卷周围，亮度极高",
