...
请用50字以内的一句话描述，必须包含'烟雾'一词，禁止出现'导弹'、'爆炸'、'火球'、'冲击波'等词汇。"""
    ...
    def get_safe_default_description(self, event_key):
        ...
        defaults = {
            "missile_appearance": [
                "一枚细长导弹从画面边缘飞入，朝目标方向飞行，与目标区域尚有距离。",
                "画面显示一枚导弹正在接近目标，可见其流线型弹体和轻微尾焰。",
                "一枚高速飞行的导弹刚进入视野，目标区域尚远。"
            ],
            "missile_impact": [
                "导弹尖端与目标区域发生接触，伴有瞬间的冲击波和少量扬尘。",
                "目标区域与导弹发生碰撞，可见局部尘土和轻微冲击波痕迹。",
                "导弹命中目标区域，发生物理接触，无大规模爆炸。"
            ],
            "explosion": [
                "明亮的橙红色火球瞬间爆发，迅速扩散，冲击波席卷周围环境。",
                "画面出现剧烈爆炸，出现巨大的火球和浓烟，亮度极高。",
                "目标区域发生猛烈爆炸，伴随巨大火光和强大的能量释放。"
            ],
            "explosion_end": [
                "爆炸产生的灰色烟雾逐渐散去，目标区域呈现严重损毁状态。",
                "浓烟开始消散，显示出目标区域的损毁情况，爆炸活动基本停止。",
                "爆炸效果已趋于平息，残余烟雾笼罩受损区域，无明显火焰。"
            ]
        }
        
        # 获取对应 event_key 的默认描述列表
        default_descriptions = defaults.get(event_key)
